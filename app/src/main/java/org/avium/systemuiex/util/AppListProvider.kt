...
object AppListProvider {
    private val romanizer by lazy {
        android.icu.text.Transliterator.getInstance("Han-Latin; Latin-ASCII")
    }

    @Synchronized
    private fun sortKey(label: CharSequence): String =
        romanizer.transliterate(label.toString().trim()).lowercase(java.util.Locale.ROOT)

    val appComparator: Comparator<AppInfo> = compareBy<AppInfo> { it.initial == "#" }
        .thenBy { it.sortKey }.thenBy { it.packageName }.thenBy { it.selectionKey }

    private fun profileType(context: Context, user: android.os.UserHandle): String {
        val users = context.createContextAsUser(user, 0)
            .getSystemService(android.os.UserManager::class.java)
        return when {
            users.isCloneProfile -> android.os.UserManager.USER_TYPE_PROFILE_CLONE
            users.isPrivateProfile -> android.os.UserManager.USER_TYPE_PROFILE_PRIVATE
            else -> android.os.UserManager.USER_TYPE_FULL_SYSTEM
        }
    }
    ...
    private fun resolveAvailableApp(context: Context, selection: String): AppInfo? {
        ...
        return AppInfo(
            ...
            profileType = profileType(context, user),
            sortKey = sortKey(activity.label),
        )
    }
